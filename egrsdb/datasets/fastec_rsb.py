...
def get_fastec_rolling_shutter_blur_dataset(
    root,
    blur_accumulate,
    events_moment,
    ...
    info(f"get_fastec_rolling_shutter_blur_dataset:")
...
def _get_fastec_rs_dataset(
    ...
    train = FastecRollingShutterBlurDataset(
        root=root,
        blur_accumulate=blur_accumulate,
    ...
    test = FastecRollingShutterBlurDataset(
        root=root,
        blur_accumulate=blur_accumulate,
        input_rs_blur_frame_timestamp=input_rs_blur_frame_timestamp,
        output_gs_sharp_frame_indexes=output_gs_sharp_frame_indexes,
...
class FastecRollingShutterBlurDataset(Dataset):
    def __init__(
        self,
        root,
        blur_accumulate,
        ...
        super(FastecRollingShutterBlurDataset, self).__init__()
    ...
    def _walk(self):
        ...
        for folder in folders:
            ...
            elif folder.endswith(f"blur-frames-H260-W346-blur-accumulate-{self.blur_accumulate}"):
        ...
        for gs_sharp_folder, event_folder, rolling_blur_folder, rolling_sharp_folder in zip(
            ...
            # |-- 190206_Tuebingen_000104
            # |-- 190206_Tuebingen_000104-blur-frames-H260-W346-blur-accumulate-260
            # |-- 190206_Tuebingen_000104_fps5000-resize-frames-H260-W346
            # |-- 190206_Tuebingen_000104_fps5000-resize-frames-H260-W346-events
            # |-- 190206_Tuebingen_000104_fps5000-resize-frames-H260-W346-rolling
            assert gs_sharp_folder[:23] == event_folder[:23] == rolling_blur_folder[:23] == rolling_sharp_folder[:23]
            video_name = gs_sharp_folder[:23]
    ...
    def _walk_video(self, gs_sharp_folder, event_folder, rolling_blur_folder, sharp_rolling_folder):
        ...
        # info(f"  gs_sharp_frames: {len(gs_sharp_frames)}")
        # info(f"  events: {len(events)}")
        # info(f"  rolling_sharp_frames: {len(rolling_sharp_frames)}")
        # info(f"  rolling_blur_frames: {len(rolling_blur_frames)}")
        ...
        for i in range(len(rolling_blur_frames)):
            # e.g., 190206_Tuebingen_000023-blur-frames-H260-W346-blur-accumulate-260/0000260-0000520.png
            ...
            # Check
            if DEBUG:
                gs_sharp_folder_path = join(self.root, gs_sharp_folder)
                event_folder_path = join(self.root, event_folder)
                info(f"  gs_sharp_folder_path: {gs_sharp_folder_path}")
                info(f"  event_folder_path   : {event_folder_path}")
                gs_sharp_count = len(listdir(gs_sharp_folder_path))
                event_count = len(listdir(event_folder_path))
                assert gs_sharp_count == event_count + 1, f"{gs_sharp_folder_path} != {event_folder_path} + 1"
                assert len(items_events) == len(items_sharp)
                rolling_start = begin_rs_index
                rolling_end = end_rs_index
                event_0_name = _file_name(items_events[0])
                event_n_name = _file_name(items_events[-1])
                sharp_0_name = _file_name(items_sharp[0])
                sharp_n_name = _file_name(items_sharp[-1])
                assert int(event_0_name) == int(sharp_0_name), f"{event_0_name} != {sharp_0_name}"
                assert int(event_n_name) == int(sharp_n_name), f"{event_n_name} != {sharp_n_name}"
                assert int(event_0_name) == int(rolling_start) - 260, f"{event_0_name} != {rolling_start} - 260"
                assert int(event_n_name) == int(rolling_end) - 1, f"{event_n_name} != {rolling_end} - 1"
            # generate an item
